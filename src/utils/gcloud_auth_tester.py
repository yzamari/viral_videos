...
class GCloudAuthTester:
    ...
    def _test_google_ai_studio_access(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                veo_models = [m for m in models_data.get('models', []) if 'veo' in m.get('name', '').lower()]
    ...
    def _analyze_auth_results(self) -> Dict[str, Any]:
        ...
        if critical_failures:
            ...
            status_message = f"Some services failed but app can still run with limitations"
        else:
        ...
        if self.test_results.get('vertex_ai', {}).get('status') == 'FAILED':
            recommendations.append("🔑 Enable Vertex AI API and check project configuration")
    ...
    def print_auth_report(self):
        ...
        print(f"📊 Tests: {analysis.get('successes', 0)} passed, {analysis.get('failures', 0)} failed")
