...
def install_gcloud():
    ...
    if system == "darwin":  # macOS
        ...
        if not success:
            ...
            print_status(
                "Please install manually: https://cloud.google.com/sdk/docs/install",
                "warning")
            return False
    else:
...
def setup_application_default_credentials() -> bool:
    ...
    success, _, error = run_command(
        ["gcloud", "auth", "application-default", "login"], capture_output=False, timeout=300)
    if not success:
...
def update_env_with_project_settings(
        env_vars: Dict[str, str], project_id: str, location: str) -> Dict[str, str]:
...
def main():
    ...
    if not check_gcloud_installed():
        if not install_gcloud():
            print_status(
                "Please install Google Cloud SDK manually and run this script again",
                "error")
            sys.exit(1)
