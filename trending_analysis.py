...
class TrendingAnalyzer:
    ...
    def analyze_trending(
        ...
        """
        logger.info(
            f"🔥 Starting trending analysis for: {
                topic or 'general trending'} (last {days_back} days)")
        ...
        try:
            ...
            logger.info(
                f"🎯 Analysis complete! {
                    len(analyses)} videos analyzed from last {days_back} days")
            return report
    ...
    def _video_to_dict(self, video: TrendingVideo) -> Dict:
        ...
        return {
            ...
            'title': video.title,
            'description': video.description[:200] + "..." if video.description and
            len(video.description) > 200 else video.description,
            'url': str(video.url),
        ...
        return {
            'content_strategy': {
                'recommended_themes': [
                    theme for theme,
                    count in top_themes],
                'viral_score_benchmark': f"{
                    avg_viral_score:.2f}",
                'engagement_benchmark': f"{
                    avg_engagement:.4f}",
                'top_success_factors': [
                    factor for factor,
                    count in top_factors]},
            'optimization_tips': [
                f"Focus on {
                    top_themes[0][0]} content (appears in {
                    top_themes[0][1]} trending videos)" if
                top_themes else "Create engaging content",
                f"Target {
                    avg_engagement:.2%} engagement rate or higher",
                f"Aim for viral score above {
                    avg_viral_score:.2f}",
                "Use trending keywords in titles and descriptions",
                "Create strong hooks in first 3 seconds"],
            'trending_patterns': {
                'most_viral_category': insights.get(
                    'insights',
                    {}).get(
                    'best_performing_category',
                    'Unknown'),
                'optimal_duration': insights.get(
                    'insights',
                    {}).get(
                    'optimal_duration',
                    'Unknown'),
                'common_hooks': insights.get(
                    'insights',
                    {}).get(
                    'common_hooks',
                    [])}}
...
def main():
    ...
    parser.add_argument("--days-back", "-d", type=int, default=6,
                        help="Number of days back to search (default: 6)")
