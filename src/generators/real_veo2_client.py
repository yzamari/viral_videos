...
class RealVeo2Client:
        ...
        try:
            ...
            if "429" in error_msg or "RESOURCE_EXHAUSTED" in error_msg:
                ...
                try:
                    ...
                    if operation.done and hasattr(operation, 'response') and hasattr(operation.response, 'generated_videos'):
                        for generated_video in operation.response.generated_videos:
                            try:
                                ...
                                if os.path.exists(output_path):
                                    file_size = os.path.getsize(output_path) / (1024 * 1024)
                                    logger.info(f"🎉 Retry SUCCESS! Real Veo-2 video saved: {output_path} ({file_size:.1f} MB)")
                                    return output_path
                            except Exception:
                                continue
    ...
    def _create_baby_animals_scene_1(self, output_path: str, width: int, height: int, fps: int, duration: float):
        ...
        try:
            ...
            cmd = [
                'ffmpeg', '-y',
                '-f', 'lavfi', '-i', f'color=lightblue:size={width}x{height}:duration={duration}:rate={fps}',
                '-vf', f'drawtext=text=\'Scene 1 - Baby & Animals\':fontcolor=white:fontsize=30:x=(w-text_w)/2:y=(h-text_h)/2',
    ...
    def _create_baby_animals_scene_2(self, output_path: str, width: int, height: int, fps: int, duration: float):
        ...
        try:
            ...
            cmd = [
                'ffmpeg', '-y',
                '-f', 'lavfi', '-i', f'color=lightgreen:size={width}x{height}:duration={duration}:rate={fps}',
                '-vf', f'drawtext=text=\'Scene 2 - Interaction\':fontcolor=white:fontsize=30:x=(w-text_w)/2:y=(h-text_h)/2',
                '-c:v', 'libx264', '-preset', 'ultrafast', '-crf', '23',
                '-pix_fmt', 'yuv420p', '-t', str(duration),
                output_path
            ]
    ...
    def _create_baby_animals_scene_3(self, output_path: str, width: int, height: int, fps: int, duration: float):
        ...
        try:
            ...
            cmd = [
                'ffmpeg', '-y',
                '-f', 'lavfi', '-i', f'color=lightyellow:size={width}x{height}:duration={duration}:rate={fps}',
                '-vf', f'drawtext=text=\'Scene 3 - Happy Moment\':fontcolor=darkblue:fontsize=30:x=(w-text_w)/2:y=(h-text_h)/2',
                '-c:v', 'libx264', '-preset', 'ultrafast', '-crf', '23',
                '-pix_fmt', 'yuv420p', '-t', str(duration),
                output_path
            ]
    ...
    def _create_premium_fallback_clip(self, prompt: str, duration: float, clip_id: str) -> str:
        ...
        try:
            ...
            cmd = [
                'ffmpeg', '-y',
                '-f', 'lavfi',
                '-i', f'color={color1}:size={width}x{height}:duration={duration}:rate={fps}',
                '-f', 'lavfi', 
                '-i', f'color={color2}:size={width}x{height}:duration={duration}:rate={fps}',
                '-filter_complex', 
                f'[0][1]blend=all_mode=screen:all_opacity=0.5,scale={width}:{height}:flags=lanczos,drawtext=fontfile=/System/Library/Fonts/Arial.ttf:text=\'Premium AI Video\\n{prompt[:50]}...\':fontcolor=white:fontsize=32:x=(w-text_w)/2:y=(h-text_h)/2:box=1:boxcolor=black@0.5:boxborderw=5',
