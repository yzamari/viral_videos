...
class EnhancedOrchestratorAgent:
    ...
    def _create_master_timeline(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"📋 Master Timeline: {
                self.target_clips} scenes × {
                self.clip_duration}s = {
                self.total_video_duration}s")
        return timeline
    ...
    def _orchestrate_director_agent(self, api_key: str, master_timeline: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"✅ Director Agent complete: {
                script_data['word_count']} words, {
                script_data['scenes_count']} scenes")
        return script_data
    ...
    def _orchestrate_video_agent(self, api_key: str, script_data: Dict[str, Any],
        ...
        logger.info(
            f"✅ Video Generator Agent complete: {
                video_data['clip_count']} clips, {
                video_data['total_duration']}s")
        return video_data
    ...
    def _create_generic_scene_prompt(self, scene_script: Dict[str, Any], scene: Dict[str, Any]) -> str:
        ...
        if scene['narrative_role'] == 'hook_attention_grabber':
            return f"Attention-grabbing opening scene related to: {
                self.topic}. Dynamic, engaging action that immediately hooks viewers."
        elif scene['narrative_role'] == 'main_action_peak':
            return f"Peak action scene showcasing the main story of: {
                self.topic}. Maximum visual interest and engagement."
        elif scene['narrative_role'] == 'conclusion_cta':
            return f"Concluding scene that wraps up the story about: {
                self.topic}. Satisfying resolution with call-to-action energy."
        else:
